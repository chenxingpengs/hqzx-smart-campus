...
async function ensureCacheDir(): Promise<string> {
  ...
  const dirPath = `${basePath}/${IMAGE_DIR}`
  ...
  try {
    ...
    try {
      ...
      console.error('创建图片缓存目录失败:', mkdirErr)
    }
  }
...
export async function getCachedImage(imageUrl: string | undefined | null): Promise<string> {
  if (!imageUrl)
    return ''
  ...
  try {
    ...
    if (cachedInfoStr) {
      const cachedInfo: ImageCacheInfo = JSON.parse(cachedInfoStr)
      ...
      if (!isExpired) {
        ...
        await deleteCachedImage(imageUrl)
      }
    }
  }
  catch (e) {
    console.error('读取图片缓存信息失败:', e)
  }
    ...
    if (cachedInfoStr) {
      ...
      try {
        fs.unlink({
          ...
          success: () => {},
          fail: (err) => {
            console.error('删除图片缓存文件失败:', err)
          },
        })
      }
      catch (e) {
        console.error('删除图片缓存文件异常:', e)
      }
...
export function clearAllImageCache(): void {
  try {
    ...
    console.error('清除所有图片缓存失败:', e)
  }
}

export async function batchGetCachedImages(urls: string[]): Promise<Record<string, string>> {
  const result: Record<string, string> = {}
  const promises = urls.map(async (url) => {
    if (url) {
      result[url] = await getCachedImage(url)
    }
  })
